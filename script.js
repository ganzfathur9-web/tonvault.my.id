...
let defaultCustomAccounts = {
  ...
  "Mike": { pass: "Mike", rank: "don" },
  "Nayi11": { pass: "Nayi11", rank: "Bisnis" },
};
...
function quickApproveTx(txId) {
  ...
  if (tx) {
    ...
    if (isFinalized && !isTopAdmin(userRank)) {
      showToast("ACCESS DENIED", "Only Admins/Moderators can modify completed transactions!", "error");
      return;
    }

    // TAHAP 1: Jika status masih PENDING -> Pindahkan ke WAITING RELEASE (Uang ditahan, stok terkirim)
    if (tx.status === 'Pending') {
      ...
      sendDiscordWebhook(
        ORDERS_WEBHOOK_URL, 
        "⏳ ORDER PROCESSED & WAITING RELEASE", 
        `TXID **${tx.id}** has been processed by **${tx.processed}**. Stock shipped, awaiting final balance release!`, 
        [
          { name: "Buyer", value: tx.buyer, inline: true },
          { name: "Held Balance", value: "$" + tx.total.toLocaleString(), inline: true }
        ], 
        16763904
      );
      ...
      return; // Stop di sini agar tidak langsung ter-release
    }

    // TAHAP 2: Jika status sudah WAITING RELEASE -> Rilis uang masuk ke Kas Brangkas (Vault Balance)
    ...
    sendDiscordWebhook(
      ORDERS_WEBHOOK_URL, 
      "🟢 TRANSACTION RELEASED & CREDITED", 
      `TXID **${tx.id}** released by **${tx.processed}**. Balance of $${tx.total.toLocaleString()} officially credited to vault!`, 
      [
        { name: "Buyer", value: tx.buyer, inline: true }, 
        { name: "Total Credited", value: "$" + tx.total.toLocaleString(), inline: true }
      ], 
      3066993
    );
...
function renderReleaseOutstanding() {
  ...
  // Hanya mengambil transaksi yang berstatus "Waiting Release"
